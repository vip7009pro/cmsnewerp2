...
- [x] Đợt 5: rà soát parity `UserManager` / `DeptManager` (Quản lý Phòng Ban & Hồ sơ Nhân sự) + sửa lỗi mobile 3 bảng — 2026-09-20:
  - `PrecisionDeptMainForm` / `PrecisionDeptSubForm` / `PrecisionDeptPosForm`: 3 khoá chính `MAINDEPTCODE` / `SUBDEPTCODE` / `WORK_POSITION_CODE` bị `readOnly` cứng trong khi backend `insertmaindept`/`insertsubdept`/`insertworkposition` nhận mã do client gửi (không phải identity) ⇒ chức năng THÊM MỚI gửi mã `0` và luôn lỗi. Fix: `readOnly={!!code}`, chỉ khoá khi sửa bản ghi đã có.
  - `DeptManager.tsx`: chuỗi tải 3 cấp chỉ đồng bộ khi chưa chọn bộ phận ⇒ sau khi xoá/sửa cấp cha, bảng cấp 2/3 vẫn hiện dữ liệu cũ. Fix: `handleLoadMainDept → handleLoadsubDept → loadWorkPosition` luôn tải đủ 3 cấp, giữ dòng đang chọn bằng `kq.find(code) ?? kq[0]`, reset sạch cấp con khi danh sách cha rỗng.
  - `DeptManager.tsx`: thêm `validateForm()` theo từng cấp trước Add/Update + hộp thoại xác nhận trước khi xoá (giữ nguyên `checkBP`).
  - Mobile `DeptManager` (lỗi người dùng báo — chỉ thấy 2 bảng + header bảng 3, không cuộn được): do `.component_element &` (specificity 0,2,0) đè block `@media` (0,1,0) ⇒ vẫn `overflow:hidden` + `max-height:100%` + `flex-basis:0`. Fix: lặp block mobile **bên trong** `.component_element &` kèm `!important`; bỏ `flex:1 1 0px` ở `__triGrid`/`__panel`; hạ chiều cao bảng mobile `420px → 320px`; `QuanLyPhongBanNhanSu.scss` bỏ `flex-basis:0`/`min-height:100vh` cho `.tabs-container`/`.tab-content`/`.tab-pane` để `.component_element` cuộn được toàn bộ 3 bảng. Áp dụng cùng cách sửa cho `PrecisionUserManager.scss`.
  - `UserManager.tsx`: bản refactor đã làm mất `loadModels()` của backup ⇒ Train Face / Check Face luôn lỗi model. Fix: `ensureFaceModels()` cache module-scope nạp `ssdMobilenetv1` + `faceRecognitionNet` + `faceLandmark68Net` từ `/models`, gọi khi mount và `await` trước mỗi lần detect; thêm guard chọn nhân viên + thông báo lỗi nêu rõ đường dẫn ảnh/model.
  - `UserManager.tsx`: `f_addEmployee`/`f_updateEmployee` trả **chuỗi lỗi** nhưng bị bỏ qua ⇒ thêm/sửa thất bại im lặng. Fix: đọc kết quả, báo `Swal` lỗi, chỉ đóng modal khi thành công; `loadEmplInfo(showNotice)` tránh bắn 2 popup liên tiếp.
  - `UserManager.tsx`: `createNewUser()` xoá trắng `CTR_CD` trong khi `insertemployee` ghi `ZTBEMPLINFO.CTR_CD = DATA.CTR_CD` ⇒ nhân viên mới không JOIN được với phòng ban/vị trí. Fix: `CTR_CD: getCtrCd()` + fallback ngay trước khi insert; mặc định `WORK_POSITION_CODE` theo `workpositionload` thay vì số `1` cứng.
...
- [x] Đợt 8: Mobile cho `PrecisionPoManager` (Kinh doanh → Quản lý PO v2) — 2026-09-20:
  - Vấn đề người dùng báo: 6 widget KPI quá to (mỗi widget chiếm gần nửa màn hình, xếp dọc 6 hàng); bộ lọc "Bộ Lọc Đơn Hàng" chiếm cột trái nên đẩy bảng PO data sang phải và teo còn một nửa.
  - `PrecisionPoManager.tsx`: thêm `isMobile` (`matchMedia("(max-width:768px)")` + listener `change`, đúng convention repo) ⇒ **conditional rendering theo viewport**; `filterCollapsed` khởi tạo theo `innerWidth <= 768` và tự đồng bộ lại khi viewport đổi (mobile mặc định ĐÓNG bộ lọc để bảng chiếm trọn chiều ngang); render `po-filter-backdrop` (mobile) để bấm ra ngoài là đóng bộ lọc.
  - `PrecisionPoKpiGrid.tsx`: thêm prop `compact` + `CompactKpiItem[]`; khi compact render **2 hàng × 3 cột**, mỗi ô **2 dòng** (nhãn 8.5px + giá trị 12.5px, unit 8px). Hàng 1 = số lượng (Tổng ĐH / Đã giao / Tồn PO), hàng 2 = số tiền (Tổng giá trị / Giá trị giao / Giá trị còn lại) — đúng thứ tự KPI 1→6, tiền dùng `maximumFractionDigits: 0` cho gọn; `title` giữ đủ ngữ nghĩa cho desktop-like tooltip.
  - `PrecisionPoFilterPanel.tsx`: nhận `isMobile`, nút header đổi `MdChevronLeft` → `MdClose` (đóng bộ lọc).
  - `PrecisionPoToolbar.tsx`: nhận `isMobile`, các nút phụ thêm class `is-icon-only` + **không render `<span>` nhãn** trên mobile (giữ `title`); ẩn `toolbar-divider` giữa nhóm action.
  - `PrecisionPoManager.scss`: thêm `.po-kpi-grid--compact` (ngoài media query ⇒ không phụ thuộc specificity) và block `@media (max-width:768px)` ở cuối `.precision-po-manager`: bộ lọc `position:absolute; inset:0; z-index:40` (float phủ workspace, KHÔNG đẩy bảng), `po-filter-backdrop` `z-index:35`, `.po-main-workspace` bỏ `max-height: calc(100% - 95px)` (nếu giữ sẽ cắt mất ~9px footer), toolbar `count-label`/divider ẩn + ẩn quick-search full width, nút phụ vuông 30px.
  - Kết quả đo ở viewport 393×850: KPI `grid-template-columns` = 3 × 121.66px, đúng 2 hàng (card 41px, không giá trị nào bị cắt), grid cao 88px (trước đây ~6 hàng); bộ lọc `position:absolute` cùng `x/width` với bảng → bảng full width 377px; toolbar 7 nút gọn trong 1 hàng (79px cả 2 dòng), `overflowXPx = 0`.
  - Kiểm chứng desktop 1440px không hồi quy: KPI 6 cột 1 hàng (53px), bộ lọc `position:static` rộng 240px không đè bảng (filter right 254 < table x 262), `count-label`/divider vẫn hiện.
  - **Fix vòng 2 (người dùng báo nút phụ mất style, "toàn nút đen trắng")**: `.is-icon-only { width:30px; padding:0 }` ép nút còn 30px nhưng nhãn `<span>` **vẫn render** ⇒ chữ tràn khỏi nút (`scrollWidth > clientWidth` ở "Tạo Invoice / Phê duyệt / Pivot / Xuất Excel"), icon bị co còn 0px ⇒ nhìn như nút trắng trơn. Fix: **conditional rendering** `{!isMobile && <span>…</span>}` cho cả 6 nút phụ (giữ `title` cho tooltip/a11y) + rule bảo hiểm `.is-icon-only span { display:none }` + tô accent theo hành động (`btn-action-edit` `#eff6ff/#bfdbfe`, `btn-action-invoice` & `-success` `#ecfdf5/#a7f3d0`, `btn-action-danger` `#fff1f2/#fecdd3`, `btn-action-pivot` `#f5f3ff/#ddd6fe`, `btn-action-excel` `#ecfdf5/#a7f3d0`).
  - Xác minh trên UI thật (`/kinhdoanh/pomanager`, viewport 393×850): 7 nút gọn **1 hàng**, `overflowXPx = 0`, mỗi nút phụ 30×30, icon SVG đủ 15px đúng màu accent, nhãn đã ẩn; bộ lọc float `position:absolute` cùng `x/width` với bảng (377px) ⇒ bảng full width, có nút đóng (X) và backdrop.
  - Xác minh: get_errors 0 lỗi trên 5 file; `npm run build` (vite production) `✓ built in 1m 15s` EXIT=0; đo layout trực tiếp trên dev server 3001 bằng `await import()` file SCSS rồi inject markup thật (không cần backend).

- [x] Đợt 9: Mobile cho `InvoiceManager` (Kinh doanh → Quản lý Invoices) — 2026-09-20:
  - Vấn đề người dùng báo: 2 widget "Delivered QTY / Delivered Amount" nằm trong bộ lọc nên bị chôn theo sidebar; bộ lọc chiếm cột trái đẩy bảng sang phải; toolbar AGTable bị xuống dòng nhìn xấu.
  - `InvoiceManager.tsx`: thêm `isMobile` (`matchMedia("(max-width:768px)")` + listener `change`) ⇒ **conditional rendering theo viewport**; `filterCollapsed` khởi tạo theo `innerWidth <= 768` + effect đồng bộ khi viewport đổi (mobile mặc định ĐÓNG bộ lọc); render `stitch-inv__filter-backdrop` khi mobile.
  - `PrecisionInvoiceKpiBar.tsx` (MỚI): tách 2 widget ra khỏi bộ lọc thành **KPI bar compact 1 hàng** (`grid-template-columns: repeat(2, minmax(0,1fr))`, mỗi ô 2 dòng: nhãn 8.5px + giá trị 13px mono + unit 8px), đặt trên cùng trang; tiền dùng `maximumFractionDigits: 0`, `title` giữ giá trị đầy đủ.
  - `PrecisionInvoiceFilterPanel.tsx`: prop `hideKpiSummary` (mobile không render lại `stitch-inv__kpi-summary` trong sidebar, desktop giữ nguyên) + prop `isMobile`/`onClose` với nút đóng `stitch-inv__sidebar-close` (FiX).
  - `PrecisionInvoiceToolbar.tsx`: nhận `isMobile`, nhãn nút toggle bộ lọc `Show/Hide` → `Lọc`.
  - `PrecisionInvoiceManager.scss` (trước đây **không có bất kỳ media query nào**): thêm `&__kpi-bar`, `&__kpi-chip*`, `&__sidebar-close`, `&__sidebar-header-actions` và block `@media (max-width:768px)` ở cuối `.stitch-inv`: `&__workspace { position: relative }`, `&__sidebar { position:absolute; inset:0; z-index:40 }` (float, không đẩy bảng), `&__filter-backdrop` z-index 35, `&__sidebar--collapsed { display:none }` **khai báo lại**, `&__toolbar-left { flex-wrap:nowrap; overflow-x:auto; min-width:0 }` + nút `flex:0 0 auto; white-space:nowrap` + ẩn scrollbar, ẩn `&__toolbar-right`.
  - Đo trên UI thật (tab "Quản lý Invoices", dev server 3001): KPI bar trên cùng 393×51 với 2 chip 185.5px/hàng, giá trị lớn thực tế (`445,810 EA`, `$11,331,644 USD`) **không bị clip**; toolbar **8 nút nằm 1 hàng** (`scrollWidth 825` vs `clientWidth 377` ⇒ scroll ngang được), `toolbar-right` ẩn; bộ lọc `position:absolute` cùng `x/width/height` với `__content` (float đè bảng), bảng full width, nút đóng 26×26 hoạt động; `overflowXPx = 0`.
  - **Pitfall phát hiện**: vì panel float phủ trọn workspace (`inset:0`) nên backdrop bị che hoàn toàn ⇒ không thể "bấm nền để đóng" như `PrecisionPoManager`; buộc phải có nút đóng trong header panel. Và rule mobile `&__sidebar` cùng specificity nhưng ở sau `&__sidebar--collapsed` của desktop ⇒ phải khai báo lại trạng thái collapsed trong media query, nếu không panel sẽ luôn mở.
  - Xác minh: get_errors 0 lỗi trên 5 file; `npm run build` (vite production) `✓ built in 52.72s` EXIT=0.

- [x] Đợt 10: Mobile cho `PlanManager` (Kinh doanh → Quản lý Plan) — 2026-09-20:
  - Vấn đề người dùng báo: bộ lọc còn là dải input inline; **nút "Thêm Plan" không xuất hiện trên giao diện mobile**.
  - Nguyên nhân nút Thêm Plan biến mất: `&__tabs` là flex item với `min-width: auto`; sub-tab `Trạng thái kiểm tra Plan (Plan Status)` có `white-space: nowrap` ⇒ min-content của tabs lớn hơn bề rộng màn hình ⇒ `.precision-plan__header` (dù `flex-wrap: wrap`) tràn ngang và `.precision-plan` (`overflow: hidden`) cắt mất `&__headerRight`.
  - Fix nút Thêm Plan (2 lớp): (a) conditional rendering rút gọn nhãn sub-tab trên mobile (`Plan Status`); (b) `&__tabs { flex:1 1 auto; min-width:0; overflow-x:auto }` + `&__headerRight { flex:0 0 auto; margin-left:auto }` + `&__addBtn { flex:0 0 auto; height:30px }`.
  - `PlanManagerManageTab.tsx`: nhận `isMobile`; state `filterOpen` (effect đóng khi chuyển sang mobile); thêm nút **BỘ LỌC** (`precision-plan__gridBtn--filter`, modifier `--active`) vào `__gridActions`; render `precision-plan__filterBackdrop` + header panel (`__toolbarHead`/`__toolbarTitle`/`__toolbarClose` dùng `FiX`); toolbar bộ lọc chỉ render khi `!isMobile || filterOpen`; tự đóng panel sau khi tra cứu thành công để thấy ngay kết quả.
  - `PrecisionPlan.scss`: thêm block `@media (max-width:768px)` ở cuối `.precision-plan` + `@keyframes pp-fade-in`; mobile: `&__content { position: relative }`, `&__toolbar { position:absolute; top/left/right:0; bottom:auto; max-height:100%; z-index:40 }` co theo nội dung, các filter group thành 1 hàng (nhãn `min-width:74px`, input `flex:1 1 auto` + override `width` của `--date`/`--text`), ẩn `__filterSep`, nút Tra Cứu full width; grid toolbar `flex-wrap: nowrap` + `overflow-x:auto` + nút `flex:0 0 auto`; **ẩn `&__gridMeta`** (số dòng đã có ở footer AG Grid).
  - Đo trên UI thật `/kinhdoanh/planmanager` @393×850: header 393×43 không tràn, tabs `["Quản lý Plan","Plan Status"]`, nút Thêm Plan tại `x=291, 92×30` **trong viewport**; grid toolbar 5 nút (BỘ LỌC/EX1/EX2/PIVOT/XÓA PLAN) **1 hàng không cần scroll**; panel float 393×468 (position absolute, trùng `x/width` với `__content`) + backdrop 393×725 ⇒ **257px backdrop bấm được**; đóng panel cả bằng nút X và bằng bấm backdrop đều OK; `overflowXPx = 0`.
  - Xác minh: get_errors 0 lỗi trên 4 file; `npm run build` (vite production) `✓ built in 50.86s` EXIT=0.
  - **Fix vòng 2 (người dùng báo modal Thêm Plan xấu, chữ xuống dòng đẩy nội dung xuống)**: `.pp-modal` trước đây **không có media query nào** ⇒ mobile bị flex bóp: `.pp-modal__headerLeft` (flex item cạnh `__headerRight` ~315px) co còn **88px** ⇒ title **8 dòng** + subtitle **9 dòng** (header cao **316px**); nút X bị đẩy ra ngoài modal (`x=423` > modal right 377, không bấm được); `.pp-manual__info` bị `__infoRight { white-space:nowrap }` bóp `__infoLeft` thành **18 dòng** (info cao **329px**).
...
- [x] Đợt 12: Mobile cho tab Amazon + modal Upload AMZ (module YCSX) — 2026-09-20:
  - **Phát hiện dead code**: user yêu cầu tối ưu `ycsxmanager/TraAMZ/TraAMZ.tsx` nhưng file này **không được render ở đâu** — chỉ được import bởi `YCSXManager.backup.tsx`, và file backup đó cũng không được route nào dùng (grep toàn `src/**` chỉ ra 3 file: TraAMZ.tsx, TraAMZ.scss, YCSXManager.backup.tsx). Tab Amazon đang chạy thật là `PrecisionYCSX/PrecisionAmzTab.tsx` (root `className="precision-ycsx"`), và nó khớp đúng 2 triệu chứng user mô tả: có 4 KPI widget + `isFilterHidden` **có state nhưng không có nút toggle nào** ⇒ bộ lọc luôn cố định. ⇒ Đã tối ưu component đang chạy, KHÔNG sửa file chết (tránh sửa code không kiểm chứng được).
  - `PrecisionAmzTab.tsx`: thêm `isMobile`; `isFilterHidden` khởi tạo theo `innerWidth <= 768` + effect; **ẩn khối `__kpiGrid` (4 card) trên mobile**; render `precision-ycsx__filterBackdrop`; thêm nút đóng `__filterClose` (`FiX`) vào `__filterHeader`; thêm modifier `__gridToolbar--compact` + class `__toolBtn--filterToggle` cho nút toggle; **ẩn nhóm input `Offset X/Y`** trên mobile.
  - `PrecisionAmzAddModal.tsx`: thêm `isMobile`; rút gọn tiêu đề (`NHẬP DỮ LIỆU AMZ`) + ẩn subtitle; **chuyển inline style sang dạng mobile** (container, `modal-body padding`, grid `1.2fr 1.2fr 2fr` → `repeat(2,…)`, box thông tin `gridColumn: span 2`, banner upload xếp dọc, nhóm nút `flexWrap`, `modal-agtable-wrapper minHeight 380 → 220`) + rút gọn text (`Đã nạp`, `Kiểm tra trùng`, `Upload`, `XEM TRƯỚC AMZ`) + ẩn hint footer.
  - **Tái sử dụng CSS**: `PrecisionYCSX.scss` **không phải sửa** — block mobile đợt 11 (filter float, `__filterClose`, `__gridToolbar--compact`, `.precision-ycsx-modal-container`) áp dụng luôn vì `PrecisionAmzTab` dùng chung root `.precision-ycsx`.
  - Kết quả @393×850: KPI ẩn; filter `position:absolute` trùng `x/width` với mainBody (backdrop còn **258px bấm được**, đóng được bằng cả X và bấm nền); toolbar **5 nút 1 hàng** (30×30 icon-only, offset inputs ẩn); modal upload: header 45px, grid 2 cột, lưới preview 339×417, body **không cần scroll**, footer 1 hàng; `overflowXPx = 0`. Desktop 1440×900 không hồi quy: KPI 4 card, filter `static` 250px, toolbar không compact, nhãn nút + offset inputs đầy đủ.
  - Xác minh: get_errors 0 lỗi trên 2 file; `npm run build` (vite production) `✓ built in 55.52s` EXIT=0.
  - **Fix vòng 3 (user yêu cầu toolbar mobile hiện CHỮ thay vì icon-only, vì đã scroll ngang được)**: thay chiến lược "ép icon vuông 30px" bằng **nhãn ngắn theo `isMobile`** — `PrecisionYCSXToolbar.tsx` thêm helper `label(short, full)` + class `--keepLabel` cho 16 nút, đồng thời 4 nút lock/unlock (trước icon-only mọi viewport) nay có nhãn ngắn trên mobile (`Khóa` / `Mở` / `Khóa VL` / `Mở VL`). Nhãn mobile: `Lọc`, `+ YCSX`, `Sửa`, `Xóa`, `Đóng lệnh`, `Chờ duyệt`, `In YCSX`, `Bản vẽ`, `Duyệt`, `EX1`, `EX2`, `PIVOT`. Áp dụng tương tự cho `PrecisionAmzTab.tsx` (`Lọc`, `In tem (n)`, `+ AMZ`) để 2 tab nhất quán.
  - Kết quả @393×850: YCSX toolbar **16 nút 1 hàng**, `scrollWidth 1188` vs `clientWidth 393` (scroll ngang ~3 màn), **0 nút bị tràn chữ bên trong**; AMZ toolbar 5 nút gọn trong 393px **không cần scroll**; `overflowXPx = 0`. Desktop 1440×900 không hồi quy: `compact: false`, nhãn dài đầy đủ, 4 nút lock vẫn icon-only (`iconOnlyCount: 4`).
  - **Fix vòng 2 (user báo nút EX1/EX2 "đen trắng và ríu rít vào nhau")**: nhãn nút viết dạng **text node trần** `<FiDownload /> EX1` (không bọc `<span>`) ⇒ rule `.compact .toolBtn > span { display:none }` không match ⇒ ô 30px bị nhồi cả icon lẫn chữ. Fix: bọc nhãn trong `<span>`, thêm accent `--emerald` (đồng bộ với `PrecisionYCSXToolbar`) và class `--keepLabel` để mobile **giữ nhãn ngắn** thay vì icon-only (EX1 53×30, EX2 55×30, có ngoại lệ `> span { display:inline }` specificity 4 class > 3 class của rule ẩn); tăng `gap` `__gridToolbarLeft/Right` 4px → 6px trên mobile. Kết quả @393: `[Lọc 30×30] [In tem 30×30] [⬇EX1 53×30] [⬇EX2 55×30] [+AMZ 30×30]` 1 hàng, không tràn; desktop 1440×900 vẫn hiện đủ nhãn (`flex-wrap: wrap`, toolbar không compact).
