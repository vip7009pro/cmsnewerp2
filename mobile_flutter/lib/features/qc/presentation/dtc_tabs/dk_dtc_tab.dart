...
class _DkDtcTabState extends ConsumerState<DkDtcTab> {
  ...
  void initState() {
    super.initState();
    _requestEmplCtrl.addListener(_onEmplCtrlChanged);
  ...
  void _onEmplCtrlChanged() {
    final currentVal = _requestEmplCtrl.text.trim();
    if (currentVal.isNotEmpty && currentVal != _lastCheckedEmplNo) {
      _checkEmplName(currentVal);
    } else if (currentVal.isEmpty &&
        (_emplName.isNotEmpty || _reqDeptCode.isNotEmpty)) {
      setState(() {
        _emplName = '';
        _reqDeptCode = '';
        _lastCheckedEmplNo = '';
  ...
  void _initFromUser() {
    ...
    if (auth is AuthAuthenticated) {
      final empl = _s(auth.session.user.emplNo);
      _requestEmplCtrl.text = empl;
  ...
  @override
  void dispose() {
    _requestEmplCtrl.removeListener(_onEmplCtrlChanged);
  ...
  Future<void> _loadInit() async {
    ...
    try {
      await Future.wait([_loadTestList(), _loadRecent()]);
    } finally {
  ...
  Future<void> _loadTestList() async {
    ...
    if (!mounted) return;
    setState(() {
      _testList = list.map((e) => {...e, 'SELECTED': false}).toList();
    });
  }
  ...
  List<PlutoRow> _buildRecentRows(
  ...
  Future<bool> _checkEmplName(String emplNo) async {
    final cleanEmpl = emplNo.trim();
    if (cleanEmpl.isEmpty) {
      if (mounted) {
        setState(() {
          _emplName = '';
          _reqDeptCode = '';
    ...
    _lastCheckedEmplNo = cleanEmpl;
    final body = await _post('checkEMPL_NO_mobile', {'EMPL_NO': cleanEmpl});
    if (_isNg(body)) {
    ...
    final deptCode = _s(list.first['WORK_POSITION_CODE']).trim();
    final name =
        '${_s(list.first['MIDLAST_NAME'])} ${_s(list.first['FIRST_NAME'])}'
            .trim();
    setState(() {
      _emplName = name;
      _reqDeptCode = deptCode;
    });
    return deptCode.isNotEmpty;
  }

  Future<void> _checkYcsx(String prodRequestNo) async {
    final body = await _post('ycsx_fullinfo', {
  ...
  Future<void> _checkLotNvl(String mLotNo) async {
    ...
    if (list.isEmpty) return;
    setState(() {
      _mName = '${_s(list.first['M_NAME'])} | ${_s(list.first['WIDTH_CD'])}'
          .trim();
  ...
  Future<void> _insertIncomingData(int dtcId) async {
    final auth = ref.read(authNotifierProvider);
    var empl = auth is AuthAuthenticated ? _s(auth.session.user.emplNo) : '';
    if (empl.isEmpty) {
      empl = _requestEmplCtrl.text.trim();
    }
    final body = await _post('insertIQC1table', {
      'M_CODE': _mCode,
      'M_LOT_NO': _inputNoCtrl.text.trim(),
      'LOT_CMS': _inputNoCtrl.text.trim().length >= 6
          ? _inputNoCtrl.text.trim().substring(0, 6)
          : _inputNoCtrl.text.trim(),
      'LOT_VENDOR': _lotNccCtrl.text.trim().isNotEmpty
          ? _lotNccCtrl.text.trim()
          : _lotNcc,
      'CUST_CD': _custCd,
      'EXP_DATE': '',
      'INPUT_LENGTH': 0,
  ...
  Future<void> _register() async {
    ...
    try {
      if (_checkNvl) {
        if (_mCode.trim().isEmpty && inputNo.isNotEmpty) {
          await _checkLotNvl(inputNo);
        }
        if (_mCode.trim().isEmpty) {
          _snack(
            'Không tìm thấy thông tin M_CODE từ LOT NVL "$inputNo"! Không thể đăng ký.',
          );
          return;
        }
      } else {
        if ((_gCode.trim().isEmpty || _prodReqNo.trim().isEmpty) &&
            inputNo.isNotEmpty) {
          await _checkYcsx(inputNo);
        }
        if (_gCode.trim().isEmpty || _prodReqNo.trim().isEmpty) {
          _snack(
            'Không tìm thấy thông tin G_CODE/YCSX từ PROD_REQUEST_NO "$inputNo"! Không thể đăng ký.',
          );
          return;
        }
      }

      if (_reqDeptCode.trim().isEmpty && requestEmpl.isNotEmpty) {
        await _checkEmplName(requestEmpl);
      }

      if (_reqDeptCode.trim().isEmpty) {
        _snack(
          'Mã nhân viên ($requestEmpl) không hợp lệ hoặc không có mã bộ phận! Không ghi nhận đăng ký.',
        );
        return;
      }
      ...
      if (_checkNvl) {
        ...
        if (oldId > 0) nextId = oldId;
      }

      if (nextId <= 0) {
        _snack('Lỗi: DTC_ID không hợp lệ ($nextId). Không ghi nhận đăng ký!');
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    final title = _checkNvl
        ? 'ĐKÝ TEST ĐTC NVL (IQC)'
        : 'ĐKÝ TEST ĐTC SẢN PHẨM';
                  ...
                  child: ExpansionTile(
                    ...
                    children: [
                            ...
                            onPressed: _loading
                                ...
                                : () async {
                                    final raw = await Navigator.of(context)
                                        .push<String>(
                                          MaterialPageRoute(
                                            builder: (_) => const DtcScanPage(
                                              title: 'Scan Lot/Label',
                                            ),
                                          ),
                                        );
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(
                          labelText: 'REQUEST_EMPL_NO',
                        ),
                        onChanged: (v) {
                          if (_emplName.isNotEmpty || _reqDeptCode.isNotEmpty) {
                            setState(() {
                              _emplName = '';
                              _reqDeptCode = '';
                            });
                      ...
                      ..._testList.map((e) {
                        ...
                        return CheckboxListTile(
                          ...
                          title: Text(
                            '${_s(e['TEST_NAME'])} (${_s(e['TEST_CODE'])})',
                          ),
                          subtitle: tested
                              ? const Text('Đã từng test (auto-selected)')
                              : null,
                        );
                      }),
                      const SizedBox(height: 10),
