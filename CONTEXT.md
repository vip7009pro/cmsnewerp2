...
- **Root cause màn hình trắng khi Logout**: `Login` được khai báo `React.lazy()` trong `src/api/lazyPages.ts` nhưng trong `App.tsx` lại render trần `{!globalLoginState && <Login />}` — **không có Suspense/ErrorBoundary nào phía trên**. Khi user vào thẳng app bằng token còn hạn (không đi qua màn login), chunk `Login` chưa từng được tải ⇒ đúng lúc bấm Logout thì `lazy` suspend mà không có boundary ⇒ React 18 unmount cả root ⇒ trắng màn hình cho tới khi F5 (F5 tải chunk trong lúc boot nên hiện được màn login). Vì vậy lỗi **không phải lúc nào cũng tái hiện**: nếu chunk Login đã nằm trong cache của phiên (do trước đó đã từng render) thì logout bình thường.
...
- **Fix phụ (`src/components/ErrorBoundary/ErrorBoundary.tsx`)**: nhận diện lỗi tải chunk (`Failed to fetch dynamically imported module`, `Loading chunk failed`, `ChunkLoadError`…) ⇒ tự `window.location.reload()` **1 lần / 15s** (guard bằng `sessionStorage.erp_chunk_autoreload_at`). Trước đây chunk cũ 404 sau mỗi lần deploy bắt user phải F5 thủ công.
- **`src/api/Api.ts` — `logout()` viết lại**: (1) cờ `loggingOut` chống logout trùng lặp (nút Logout, token hết hạn, `checkLicense`, socket `notification` đều có thể gọi cùng lúc); (2) bỏ `setTimeout(1000)` — xoá `userData` + hạ `loginState` **ngay trong cùng nhịp render**, không để AppRoutes chạy tiếp 1s với token đã bị vô hiệu (các interval `getchamcong` 30s / `checkWebVer` / `checkLicense` bắn request lỗi và có thể trigger logout lần 2); (3) thêm `isLoggingOut()` để chặn race “hồi sinh token”.
- **Fix race token (`src/pages/home/Home.tsx`)**: `getchamcong()` trước đây ghi `REFRESH_TOKEN` về cookie cho mọi response OK ⇒ request bắn **trước** logout nhưng về **sau** sẽ ghi lại token hợp lệ (UI đã logout nhưng F5 vào lại được app). Nay thêm điều kiện `&& !isLoggingOut()`.
- **Ghi nhận chưa sửa (cần quyết định)**: `BrowserRouter` giữ nguyên URL sâu sau khi logout (đăng nhập lại sẽ về đúng trang cũ — có thể là feature); JWT ký bằng secret hard-code `"nguyenvanhung"` và là token stateless nên logout chỉ có tác dụng phía client (token đã phát vẫn hợp lệ tới khi hết hạn); `update_socket` gọi `socket.emit` trong reducer (side-effect, bị double-invoke bởi StrictMode ở dev).
- Diagnostics sạch và `npm run build` thành công (`✓ built in 1m 41s`).
