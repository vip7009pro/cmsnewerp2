...
export const current_ver: number = getCompany() === "CMS" ? 2580 : 422;
    ...
    <div className='home'>
      ...
      <div className='homeContainer'>
        ...
        <div className='outletdiv'>
          ...
          {tabModeSwap &&
        tabs.filter(
          (ele: ELE_ARRAY) => ele.ELE_CODE !== "-1" && ele.ELE_CODE !== "NS0"
        ).length > 0 && (
          <div className="tabsdiv">
            ...
            >
              {tabs.map((ele: ELE_ARRAY, index: number) => {
                if (ele.ELE_CODE !== "-1" && ele.ELE_CODE !== "NS0") {
                  return (
                    <MyTabs.Tab
                      key={index}
                      title={`${index + 1}. ${ele.ELE_NAME}`}
                      showClose={true}
                      onClose={() => handleTabClose(index)}
                      shouldRender={true} // Có thể thay đổi thành true cho tab cụ thể nếu muốn giữ render
                    >
                      <div
                      ...
                      >
                        <Suspense fallback={<div>Loading...</div>}>
                          {menulist.filter(
                            (menu: MENU_LIST_DATA) => menu.MENU_CODE === ele.ELE_CODE
                          )[0]?.MENU_ITEM}
                        </Suspense>
                      </div>
                    </MyTabs.Tab>
