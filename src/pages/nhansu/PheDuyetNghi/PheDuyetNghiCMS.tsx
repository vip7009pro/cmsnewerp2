...
const PheDuyetNghiCMS = ({ option }: { option: string }) => {
    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          const onClick = async (pheduyet_value: number) => {
            if (pheduyet_value === 3) {
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  const apiResult = await teamManagementService.setPheDuyet(params.data.OFF_ID, pheduyet_value);
                  if (apiResult.success) {
                    const newProjects = diemdanhnhomtable.map((p) =>
                      p.OFF_ID === params.data.OFF_ID
                        ? { ...p, APPROVAL_STATUS: pheduyet_value }
                        : p
                    );
                    setDiemDanhNhomTable(newProjects);
                  } else {
                    Swal.fire("Có lỗi", "Nội dung: " + apiResult.message, "error");
                  }
                }
              });
  ...
  const loadPheDuyetNghi = useCallback(async () => {
    teamManagementService.loadPheDuyetNghi(option, {
      FROM_DATE: fromdate,
      TO_DATE: todate,
