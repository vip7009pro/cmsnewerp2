...
const TabDangKy = () => {
    ...
    const handle_xacnhan = ()=> {   
        ...
        .then(response => {            
            ...
            {
                Swal.fire("Thông báo", "Chúc mừng bạn, Xác nhận chấm công thành công !", "success");               
            }
            else
            {
    ...
    const hanlde_dangkynghi = ()=> {   
        ...
        .then(response => {            
            if(response.data.tk_status === "OK")
            {
                Swal.fire("Thông báo", "Chúc mừng bạn, đăng ký nghỉ thành công !", "success");               
            }
            else
            {
                Swal.fire("Lỗi", "Đăng ký nghỉ thất bại thất bại ! " + response.data.message, "error");
            }            
        })
        .catch(error => {
            console.log(error);
    ...
    const hanlde_dangkytangca = ()=> {
        ...
        .then(response => {            
            if(response.data.tk_status === "OK")
            {
                Swal.fire("Thông báo", "Chúc mừng bạn, đăng ký tăng ca thành công !", "success");
            }
            else
            {
                Swal.fire("Thông báo", "Đăngkys tăng ca thất bại ! " + response.data.message, "error");
            }            
        })
        .catch(error => {
            console.log(error);
  ...
  return (
    ...
    <button className='showhidebutton' onClick={()=>setQuanLyNhanSuShow(!quanlynhansuShow)}>Form đăng ký</button>
      <div className='formdangkys'>      
      {quanlynhansuShow && <div className='formnho'>
        <h3>Đăng ký nghỉ</h3>
        <div className='dangkyform'>
            <div className='dangkyinput'>             
              <div className='dangkyinputbox'>
                <label><b>Ca nghỉ:</b>
                    <select name='canghi' value={canghi}  onChange={e=> setCanNghi(Number(e.target.value))}>
                        <option value = {1}>Ca 1</option>  
                        <option value = {2}>Ca 2</option>                                      
                    </select>
                </label>                
                <label><b>Nghỉ từ ngày:</b><input type='date' value={fromdate.slice(0,10)} onChange={e => setFromDate(e.target.value)}></input></label>                
                <label><b>Nghỉ tới ngày ngày:</b> <input type='date' value={todate.slice(0,10)} onChange={e => setToDate(e.target.value)}></input></label>                
                <label><b>Kiểu nghỉ:</b> 
                    <select name='nghitype' value={nghitype}  onChange={e=> setNghiType(Number(e.target.value))}>
            ...
            <div className='dangkybutton'>
                <button className='thembutton' onClick={hanlde_dangkynghi}>Đăng ký</button>                      
                <button className='xoabutton' onClick={handlecleardangkynghi}>Clear</button>                
            </div>
          </div>         
        ...
        <div className='dangkyform'>
            ...
            <div className='dangkybutton'>
                <button className='thembutton' onClick={hanlde_dangkytangca}>Đăng ký</button>
                <button className='xoabutton' onClick={handlecleartangca}>Clear</button>                
            </div>
          </div>
        </div>}
        {quanlynhansuShow && <div className='formnho'>
        ...
        <div className='dangkyform'>
            <div className='dangkyinput'>             
              <div className='dangkyinputbox'> 
              <label><b>Kiểu nghỉ:</b> 
                    <select name='nghitype' value={confirm_type}  onChange={e=> setConfirm_Type(e.target.value)}>
                        <option value = 'GD'>Quên giờ vào</option>  
                        <option value = 'GS'>Quên giờ về</option>                                      
                        <option value = 'CA'>Quên cả giờ vào - giờ về</option>
                    </select>
                </label>   
                <label><b>Ngày quên:</b><input type='date' value={confirm_date} onChange={e => setConfirm_Date(e.target.value)}></input></label>          
                <label><b>Giờ vào-về:</b> <input type='text' placeholder='0800-1700' value={confirm_worktime} onChange={e => setConfirm_WorkTime(e.target.value)}></input></label>
                </div>              
            </div>
            <div className='dangkybutton'>
                <button className='thembutton' onClick={handle_xacnhan}>Xác nhận</button>
                <button className='xoabutton' onClick={handleclearxacnhan}>Clear</button>                
            </div>
          </div>
        </div>}
