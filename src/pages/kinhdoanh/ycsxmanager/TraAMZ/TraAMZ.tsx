...
const TraAMZ = () => { 
 const [selectionModel_INPUTSX, setSelectionModel_INPUTSX] = useState<any>([]);
  ...
  const column_lichsuinputlieusanxuat = [
    ...
    { field: "PROD_REQUEST_NO", headerName: "YCSX_NO", minWidth: 80, flex: 0.8 },
    ...
    { field: "PRINT_STATUS", headerName: "PRINT_STATUS", minWidth: 80, flex: 1 },
  ...
  function CustomToolbarLICHSUINPUTSX() {
    return (
      <GridToolbarContainer>      
        ...
        <div className="div" style={{fontSize:20, fontWeight:'bold'}}>Bảng quản lý Data AMZ</div>
      </GridToolbarContainer>
    );
  }
  const handle_loadlichsuinputlieu = ()=>
  {
    ...
    .then((response) => {
      ...
      if (response.data.tk_status !== "NG") {
        const loaded_data: AMAZON_DATA[] = response.data.data.map((element: AMAZON_DATA, index: number)=> {
          return {
            ...element,
