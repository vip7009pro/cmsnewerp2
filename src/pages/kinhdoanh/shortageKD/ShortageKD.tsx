...
const ShortageKD = () => {
  ...
  const column_shortage = [
    ...
    { field: "PO_BALANCE", headerName: "TON PO", width: 80, renderCell: (params: any)=> {
      return (
        <span style={{color:'red', fontWeight: 'bold'}}>{params.row.PO_BALANCE.toLocaleString('en-US')}</span>
      )
    } },
    { field: "TON_TP", headerName: "TON_TP", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'blue', fontWeight: 'bold'}}>{params.row.TON_TP.toLocaleString('en-US')}</span>
      )
    } },
    { field: "BTP", headerName: "BTP", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'blue', fontWeight: 'bold'}}>{params.row.BTP.toLocaleString('en-US')}</span>
      )
    } },
    { field: "TONG_TON_KIEM", headerName: "T_KIEM", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'blue', fontWeight: 'bold'}}>{params.row.TONG_TON_KIEM.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D1_9H", headerName: "D1_9H", width: 80  , renderCell: (params: any)=> {
      return (
        <span style={{color:'green', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D1_13H", headerName: "D1_13H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'green', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D1_19H", headerName: "D1_19H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'green', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D1_21H", headerName: "D1_21H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'green', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D1_23H", headerName: "D1_23H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'green', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D2_9H", headerName: "D2_9H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'blue', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D2_13H", headerName: "D2_13H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'blue', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D2_21H", headerName: "D2_21H", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'blue', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D3_SANG", headerName: "D3_SANG", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'black', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D3_CHIEU", headerName: "D3_CHIEU", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'black', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D4_SANG", headerName: "D4_SANG", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'black', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "D4_CHIEU", headerName: "D4_CHIEU", width: 80 , renderCell: (params: any)=> {
      return (
        <span style={{color:'black', fontWeight: 'bold'}}>{params.row.D1_9H.toLocaleString('en-US')}</span>
      )
    } },
    { field: "TODAY_TOTAL", headerName: "TODAY_TOTAL", width: 120 , renderCell: (params: any)=> {
      return (
        <span style={{color:'#b63dfc', fontWeight: 'bold'}}>{params.row.TODAY_TOTAL.toLocaleString('en-US')}</span>
      )
    } },
    { field: "TODAY_THIEU", headerName: "TODAY_THIEU", width: 120 , renderCell: (params: any)=> {
      return (
        <span style={{color:'#f21400', fontWeight: 'bold'}}>{params.row.TODAY_THIEU.toLocaleString('en-US')}</span>
      )
    } },    
  ...
  const handletraShortage =() => {
    ...
    .then(response => {
        ...
        if(response.data.tk_status !=='NG')
        {
          const loadeddata: ShortageData[] =  response.data.data.map((element:ShortageData,index: number)=> {
            return {
              ...element,
              PLAN_DATE : moment.utc(element.PLAN_DATE).format("YYYY-MM-DD"),              
            }
        ...
        <div className='tracuuPlan'>
          ...
          <div className='tracuuPlanTable'>
            <DataGrid
              ...
              onSelectionModelChange={(ids) => {handleShortageSelectionforUpdate(ids);}}
            />
          </div>
        </div>
      )}
