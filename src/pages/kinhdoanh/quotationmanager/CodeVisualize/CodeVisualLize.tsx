...
const CodeVisualLize = ({DATA}: {DATA : CODEDATA}) => { 
    let factor = 1;
    let standard_rect: number = 85;
    if(DATA.G_LENGTH*DATA.G_C_R+ DATA.G_LG*DATA.G_C_R >standard_rect)
    {
        factor = standard_rect*1.0/(DATA.G_LENGTH*DATA.G_C_R+ DATA.G_LG*DATA.G_C_R);
    }    
    ...
    const renderProduct = ()=> {
        let tempProductArray:ReactElement[] = [
            <RECTANGLE            
             DATA={{
                SIZE_W: (DATA.G_SG_L + (DATA.G_CG+ DATA.G_WIDTH) * (DATA.G_C-1) +  DATA.G_WIDTH + DATA.G_SG_R)*factor*1.0,
            ...
            for(let j=0;j<DATA.G_C_R;j++)
            {
                ...
                tempProductArray.push(
                    <RECTANGLE DATA={{
                        ...
                        GIATRI:'',
                        PHANLOAI_DT:'',
                        POS_X:(DATA.G_SG_L+(DATA.G_WIDTH*i+DATA.G_CG*i))*factor*1.0 ,
