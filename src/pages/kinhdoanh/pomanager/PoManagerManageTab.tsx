...
const PoManagerManageTab: React.FC = () => {
  ...
  const handleOpenNewPODialog = () => setOpenNewPODialog(true);
  const handleCloseNewPODialog = () => setOpenNewPODialog(false);
  const handleOpenNewInvoiceDialog = () => setOpenNewInvoiceDialog(true);
  const handleCloseNewInvoiceDialog = () => setOpenNewInvoiceDialog(false);
  ...
  const handle_fillsuaformInvoice = () => {
    if (clickedRow.current !== null) {
      ...
      setNewRdDate(clickedRow.current?.RD_DATE ?? "");
      setNewInvoiceQty(0);
      setNewPoNo(clickedRow.current?.PO_NO === undefined ? "" : clickedRow.current?.PO_NO);
  ...
  const handle_add_1PO = async () => {
    ...
    let recheckPrice: number =
      newcodeprice.filter((e: PRICEWITHMOQ, index: number) => {
    ...
    if (err_code === 0) {
      ...
      if (kq === "OK") {
        ...
        Swal.fire("Thông báo", "Thêm PO mới thất bại: " + kq, "error");
      }
    } else if (err_code === 1) {
  ...
  const updatePO = async () => {
    ...
    if (clickedRow.current && newpoqty < clickedRow.current.TOTAL_DELIVERED) {
      err_code = 5;
    }
    if (getCompany() !== "CMS") {
      if (clickedRow.current && Number(newpoprice) !== clickedRow.current.PROD_PRICE) {
        err_code = 6;
      }
    }
    if (err_code === 0) {
  ...
  const dataSource = new PivotGridDataSource({
    fields: [
      ...
      { caption: "PO_NO", dataField: "PO_NO", dataType: "string", summaryType: "count", area: "row" },
      { caption: "G_NAME_KD", dataField: "G_NAME_KD", dataType: "string", summaryType: "count", area: "row" },
      { caption: "PO_QTY", dataField: "PO_QTY", dataType: "number", summaryType: "sum", area: "data" },
      { caption: "PO_AMOUNT", dataField: "PO_AMOUNT", dataType: "number", summaryType: "sum", area: "data" },
    ],
    store: podatatable,
  });

  const currency =
    getGlobalSetting()?.filter((ele: WEB_SETTING_DATA, index: number) => ele.ITEM_NAME === "CURRENCY")[0]?.CURRENT_VALUE ?? "USD";
  ...
  const poDataAGTable = useMemo(
    ...
    [podatatable, showhidePivotTable]
  );
  ...
  return (
    <div className="tracuuPO">
      {sh && (
        <div className="tracuuPOform" style={{ backgroundImage: theme.CMS.backgroundImage }}>
      ...
      <div className="tracuuPOTable">
        ...
        <div className="tablegrid">{poDataAGTable}</div>
      </div>
            ...
            <div className="dangkyinput">
              ...
              <div className="dangkyinputbox">
                <label>
                  ...
                  <TextField
                    value={newpono}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewPoNo(e.target.value)}
                    size="small"
                    color="success"
                    className="autocomplete"
                    id="outlined-basic"
                ...
                <label>
                  ...
                  <TextField
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      ...
                      let tempBEP: number = newcodeprice.filter((e: PRICEWITHMOQ, index: number) => {
                        return tempQTY >= e.MOQ;
                      })[0]?.BEP ?? 0;
                ...
                <label>
                  ...
                  <TextField
                    value={newporemark}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewPoRemark(e.target.value)}
                    size="small"
                    color="success"
                    className="autocomplete"
                    id="outlined-basic"
          ...
          <div className="dangkyinput">
            ...
            <div className="dangkyinputbox">
              ...
              <label>
                ...
                <TextField
                  value={newinvoiceQTY}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewInvoiceQty(Number(e.target.value))}
                  size="small"
                  color="success"
                  className="autocomplete"
                  id="outlined-basic"
              ...
              <label>
                ...
                <TextField
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewInvoiceRemark(e.target.value)}
                  size="small"
                  className="autocomplete"
                  id="outlined-basic"
                  label="Remark"
...
export default PoManagerManageTab;
