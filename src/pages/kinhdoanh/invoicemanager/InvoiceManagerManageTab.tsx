...
const InvoiceManagerManageTab = () => {
  ...
  const [showCheckKho, setShowCheckKho] = useState(false);
  ...
  const handletraInvoice = async () => {
    ...
    if (loadeddata.length > 0) {
      ...
      for (let i = 0; i < loadeddata.length; i++) {
        invoice_summary_temp.total_delivered_qty += loadeddata[i].DELIVERY_QTY;
        invoice_summary_temp.total_delivered_amount += loadeddata[i].DELIVERED_AMOUNT;
      }
    ...
    generalQuery("loadxuatkhopo", {
      ...
      .then((response) => {
        if (response.data.tk_status !== "NG") {
          const loadeddata: XUATKHOPODATA[] = response.data.data.map((element: XUATKHOPODATA, index: number) => {
            return {
              ...element,
  ...
  const handle_add_1Invoice = async () => {
    ...
    if (err_code === 0) {
      ...
      Swal.fire("Thông báo", "NG: Số lượng giao hàng nhiều hơn PO BALANCE", "error");
    } else if (err_code === 6) {
  ...
  const handle_fillsuaformInvoice = () => {
    if (invoicedatatablefilter.current.length === 1) {
      ...
      setSelectedID(clickedRow.current?.DELIVERY_ID ?? null);
  ...
  const updateInvoice = async () => {
    ...
    if (err_code === 0) {
      ...
      if (kq === "OK") {
        ...
        Swal.fire("Thông báo", "Update Invoice thất bại: " + kq, "error");
      }
    } else if (err_code === 1) {
      Swal.fire("Thông báo", "NG: Không tồn tại PO", "error");
    } else if (err_code === 2) {
  ...
  const updateInvoiceNo = async (invoice_no_update: string) => {
    if (invoicedatatablefilter.current.length >= 1) {
      let err_code: boolean = false;
      for (let i = 0; i < invoicedatatablefilter.current.length; i++) {
        if (invoicedatatablefilter.current[i].EMPL_NO === userData?.EMPL_NO) {
                ...
                <span style={{ color: "green" }}>
                  <b>
                    {params.data.DELIVERED_AMOUNT?.toLocaleString("en-US", {
                      style: "currency",
                      currency: getGlobalSetting()?.filter((ele: WEB_SETTING_DATA) => ele.ITEM_NAME === "CURRENCY")[0]?.CURRENT_VALUE ?? "USD",
                    })}
                  </b>
                </span>
              );
  ...
  return (
      ...
      <div className="tracuuInvoice">
        ...
        <div className="tracuuInvoiceTable">{invoiceDataAGTable}</div>
          <CustomDialog
            isOpen={openDialog}
                ...
                <div className="dangkyinputbox">
                  <label>
                    ...
                    <Autocomplete
                      ...
                      className="autocomplete invoiceManageDialogField"
        ...
        {showCheckKho && (
          <div className="newinvoice">
            <div className="batchnewinvoice">
              ...
              <div className="insertInvoiceTable">{xuatkhoPOAGTable}</div>
            </div>
          </div>
        )}
      </div>
...
export default InvoiceManagerManageTab;
