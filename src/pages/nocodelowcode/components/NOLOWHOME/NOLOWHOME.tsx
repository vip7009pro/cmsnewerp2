...
const SQL_DATA_TYPES = [
  "BIT", "INT", "BIGINT", "DECIMAL", "NUMERIC", 
  "FLOAT", "CHAR", "VARCHAR", "VARCHAR(MAX)", "NCHAR", "NVARCHAR", "NVARCHAR(MAX)",
  ...
  return (
    <Box className="nocodelowcode" p={2}>
      <Grid container spacing={1}>
        ...
        <Grid item xs={12} md={5}>
          ...
          <Box display="flex" gap={1} mb={2}>
            ...
            <Button variant="contained" color="primary" onClick={handleAddForm} startIcon={<Add />}>Add</Button>
            <Button variant="contained" color="warning" onClick={handleUpdateForm} startIcon={<Edit />}>Update</Button>
            <Button variant="contained" color="error" onClick={handleLoadForm} startIcon={<Refresh />}>Refresh</Button>
          </Box>
              ...
              <TableBody>
                {forms.map(form => (
                  <TableRow key={form.FormID} selected={selectedForm?.FormID === form.FormID}>
                    ...
                    <TableCell>
                      <IconButton onClick={() => { setSelectedForm(form); setFormEdit(form); }}><Edit /></IconButton>
                      <IconButton onClick={() => handleDeleteForm(form.FormID)}><Delete /></IconButton>
                      <IconButton onClick={() => handleOpenDataDialog(form)} title="Nhập dữ liệu"><TableView /></IconButton>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
        ...
        <Grid item xs={12} md={7}>
          ...
          {selectedForm ? (
              ...
              <Box display="flex" gap={2} mb={2}>
                ...
                <FormControl size="small">
                  ...
                  <Select
                    ...
                    onChange={e => setSelectedField({ ...selectedField, IsRequired: e.target.value === 'Yes' })}
                  >
                ...
                {/* <FormControl>
                  <Checkbox
                    ...
                    onChange={e => setSelectedField({ ...selectedField, IsRequired: e.target.checked })}
                    size="small"
                  />
                  <FormLabel>Is Required</FormLabel>
                </FormControl> */}
                <Button variant="contained" color="primary" onClick={handleAddField} startIcon={<Add />}>Add</Button>
                <Button variant="contained" color="warning" onClick={handleUpdateField} startIcon={<Edit />}>Update</Button>
              </Box>
              <TableContainer component={Paper}>
                <Table size="small">
                  <TableHead>
                  ...
                  <TableBody>
                    {fields.filter(f => f.FormID === selectedForm.FormID).map(field => (
                      <TableRow key={field.FieldID} selected={selectedField.FieldID === field.FieldID}>
                        ...
                        <TableCell>
                          ...
                          <IconButton onClick={() => handleDeleteField(field.FieldID)}><Delete /></IconButton>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <Typography variant="body1" color="text.secondary">Chọn một Form để quản lý Field.</Typography>
          )}
        </Grid>
      </Grid>
      {/* Data Entry Dialog */}
      <Dialog open={showDataDialog} onClose={handleCloseDataDialog} maxWidth="lg" fullWidth>
        ...
        <DialogContent>
          <Button onClick={handleAddDataRow} variant="outlined" sx={{ mb: 2 }}>Thêm dòng</Button>
          <TableContainer component={Paper}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableRow key={row.id}>
                    {fields.filter(f => f.FormID === selectedForm?.FormID).map(field => (
                      <TableCell key={field.FieldID}>
                        <TextField
                          value={row[field.FieldID] || ''}
                          onChange={e => handleDataCellChange(rowIdx, field.FieldID, e.target.value)}
                          size="small"
                        />
                      </TableCell>
                    ))}
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveData} variant="contained">Lưu</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
