...
import { BTP_AUTO_DATA, BTP_AUTO_DATA2, BTP_AUTO_DATA_SUMMARY } from "../../../api/GlobalInterface";
...
/*     { field: "FINAL_MACHINE", headerName: "FINAL_MACHINE", width: 150 }, */
  ...
  { field: "TEMP_QTY_EA", headerName: "TON_EA", width: 50, type: 'number', cellRenderer:(params: any)=> {
    return (
      <span style={{color: 'blue', fontWeight:'bold'}}>{params.value?.toLocaleString('en-US')}</span>
    )
  }  },
...
/*     { field: "FINAL_MACHINE", headerName: "FINAL_MACHINE", width: 150 }, */
  ...
  const columns_btp_summary = [
    ...
    { field: "XA", headerName: "XA", width: 50, cellRenderer:(params: any)=> {
      return (
        <span style={{color: 'green', fontWeight:'normal'}}>{params.value?.toLocaleString('en-US')}</span>
      )
    }  },
    { field: "XB", headerName: "XB", width: 50, cellRenderer:(params: any)=> {
      return (
        <span style={{color: 'blue', fontWeight:'normal'}}>{params.value?.toLocaleString('en-US')}</span>
      )
    }  },
    { field: "TOTAL_BTP", headerName: "TOTAL_BTP", width: 80, cellRenderer:(params: any)=> {
      return (
        <span style={{color: 'red', fontWeight:'bold'}}>{params.value?.toLocaleString('en-US')}</span>
      )
    }  },
  ]
  const [columnDefinition, setColumnDefinition] = useState<Array<any>>(columns_btp_auto_2);
  ...
  const handleLoadBTPAutoData = async () => {
    ...
    setBTPData(kq.map((ele: BTP_AUTO_DATA2, index: number) => {
      return {
        ...ele,
        id: index
    }
