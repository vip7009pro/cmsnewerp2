...
interface PrecisionSxPieLossEmplProps {
  data: SX_LOSSTIME_BY_EMPL[];
}

const PrecisionSxPieLossEmpl: React.FC<PrecisionSxPieLossEmplProps> = ({ data }) => {
  ...
  const sortedData = useMemo(() => {
    ...
    return [...data].sort((a, b) => (b.TOTAL_LOSS_TIME || 0) - (a.TOTAL_LOSS_TIME || 0));
  }, [data]);

  const totalTime = useMemo(() => {
    return sortedData.reduce((sum, item) => sum + (item.TOTAL_LOSS_TIME || 0), 0);
  }, [sortedData]);

  const filteredData = useMemo(() => {
    if (!searchTerm.trim()) return sortedData;
    const term = searchTerm.toLowerCase();
    return sortedData.filter((item) => item.EMPL_NAME?.toLowerCase().includes(term));
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const pct = totalTime > 0 ? ((item.TOTAL_LOSS_TIME / totalTime) * 100).toFixed(1) : "0";
      return (
        <div className="po-cust-tooltip">
          <div className="po-cust-tooltip__title">{item.EMPL_NAME}</div>
          <div className="po-cust-tooltip__row">
            <span className="label">Thời Gian Dừng:</span>
            <span className="value">{item.TOTAL_LOSS_TIME?.toLocaleString("en-US")} phút</span>
          </div>
          <div className="po-cust-tooltip__row">
          ...
          <div className={`po-customer-chart__donut-pane ${viewMode === "chart" ? "full" : ""}`}>
            ...
            <div className="po-donut-center">
              <span className="po-donut-center__label">
                {activeItem ? activeItem.EMPL_NAME : "TỔNG DỪNG"}
              </span>
              <span className="po-donut-center__value">
                {activeItem ? formatCompact(activeItem.TOTAL_LOSS_TIME) : formatCompact(totalTime)}
              </span>
              <span className="po-donut-center__unit">
                {activeItem ? `${((activeItem.TOTAL_LOSS_TIME / (totalTime || 1)) * 100).toFixed(1)}%` : "PHÚT"}
              </span>
            </div>
          </div>
        )}
        ...
        {viewMode !== "chart" && (
          <div className={`po-customer-chart__list-pane ${viewMode === "list" ? "full" : ""}`}>
            ...
            <div className="po-cust-table-header">
              ...
              <span className="col-name">Nhân Viên</span>
            ...
            <div className="po-cust-table-body">
              {filteredData.length === 0 ? (
                ...
                filteredData.map((item, idx) => {
                  const originalIndex = sortedData.findIndex((s) => s.EMPL_NAME === item.EMPL_NAME);
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="col-balance">
                        {item.TOTAL_LOSS_TIME?.toLocaleString("en-US")} min
                      </span>
...
export default React.memo(PrecisionSxPieLossEmpl);
