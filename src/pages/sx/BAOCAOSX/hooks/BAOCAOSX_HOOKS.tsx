...
export const usehandle_loadALL_HOAN_THANH_TRUOC_HAN_RATE_BACKDATA = (fromDate: string, toDate: string, df: boolean) => {
  const [data, setData] = useState<TRUOCHAN_BACK_DATA[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);  
...
export const usehandle_loadALL_GAP_RATE_BACKDATA = (fromDate: string, toDate: string, df: boolean) => {
  const [data, setData] = useState<ALL_GAP_RATE_BACK_DATA[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);  
  ...
  const fetchData = useCallback(
    async () => {  
      setLoading(true);
      setError(null);
      let result = await f_loadALL_GAP_RATE_BACKDATA({
            FROM_DATE: df ? frd : fromDate,
            TO_DATE: df ? td : toDate,            
          }); 
      setData(result);     
...
export const usehandle_loadKT_GAP_RATE_BACKDATA = (fromDate: string, toDate: string, df: boolean) => {
  const [data, setData] = useState<KT_GAP_RATE_BACK_DATA[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);  
  ...
  const fetchData = useCallback(
    async () => {  
      setLoading(true);
      setError(null);
      let result = await f_loadKT_GAP_RATE_BACKDATA({
            FROM_DATE: df ? frd : fromDate,
            TO_DATE: df ? td : toDate,            
          }); 
      setData(result);     
...
export const usehandle_loadSX_GAP_RATE_BACKDATA = (fromDate: string, toDate: string, df: boolean) => {
  const [data, setData] = useState<SX_GAP_RATE_BACK_DATA[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);  
  ...
  const fetchData = useCallback(
    async () => {  
      setLoading(true);
      setError(null);
      let result = await f_loadSX_GAP_RATE_BACKDATA({
            FROM_DATE: df ? frd : fromDate,
            TO_DATE: df ? td : toDate,            
          }); 
      setData(result);     
...
export const usehandle_loadYCSX_GAP_RATE_BACKDATA = (fromDate: string, toDate: string, df: boolean) => {
  const [data, setData] = useState<KD_YC_GAP_RATE_BACK_DATA[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);  
  ...
  const fetchData = useCallback(
    async () => {  
      setLoading(true);
      setError(null);
      let result = await f_loadYCSX_GAP_RATE_BACKDATA({
            FROM_DATE: df ? frd : fromDate,
            TO_DATE: df ? td : toDate,            
          }); 
      setData(result);     
