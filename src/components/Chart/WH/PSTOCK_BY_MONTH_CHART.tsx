...
import { M_STOCK_BY_MONTH_DATA, P_STOCK_BY_MONTH_DATA } from "../../../pages/kho/interfaces/khoInterface";

const PSTOCK_BY_MONTH_CHART = ({ data }: { data: P_STOCK_BY_MONTH_DATA[] }) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className='custom-tooltip'>
          <p className='label'>{`${payload[0].value.toLocaleString("en-US",{minimumFractionDigits: 0, maximumFractionDigits: 0})}`}m²</p>
        </div>
      );
    }
    return null;
  ...
  const CustomLabel = ({
    ...
    return (
      ...
      >
        {data[index].PHANLOAI} : (
        {value.toLocaleString("en-US",{minimumFractionDigits: 0, maximumFractionDigits: 0})} EA
        )
      </text>
    );
  };
...
export default PSTOCK_BY_MONTH_CHART;
