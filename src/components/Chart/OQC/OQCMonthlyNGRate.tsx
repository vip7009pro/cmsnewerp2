...
const OQCMonthlyNGRate = ({
  dldata,
  processColor,
  materialColor,
}: {dldata: OQC_TREND_DATA[], processColor: string, materialColor: string}) => {
    ...
    if (active && payload && payload.length) {      
      return (
        ...
        >
          ...
          <p className='label'>
          NG RATE: {`${payload[2].value.toLocaleString("en-US",)}%`}
          </p>
  ...
  return (
    <CustomResponsiveContainer>
      ...
      >
        ...
        <XAxis dataKey='DELIVERY_YM' height={40} tick={{fontSize:'0.7rem'}}>         
...
export default OQCMonthlyNGRate;
