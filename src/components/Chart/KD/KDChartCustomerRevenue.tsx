...
import { CUSTOMER_REVENUE_DATA } from "../../../pages/kinhdoanh/interfaces/kdInterface";
...
interface ChartCustomerRevenueProps {
  data: CUSTOMER_REVENUE_DATA[];
}

const KDChartCustomerRevenue: React.FC<ChartCustomerRevenueProps> = ({ data }) => {
          ...
          <div className={`po-customer-chart__donut-pane ${viewMode === "chart" ? "full" : ""}`}>
            ...
            <div className="po-donut-center">
              <span className="po-donut-center__label">
                {activeCustomer ? activeCustomer.CUST_NAME_KD : "DOANH THU"}
              </span>
              <span className="po-donut-center__value">
                ${activeCustomer
                  ? formatCompact(activeCustomer.DELIVERY_AMOUNT)
                  : formatCompact(totalRevenue)}
              </span>
              <span className="po-donut-center__unit">
                {activeCustomer
                  ? `${((activeCustomer.DELIVERY_AMOUNT / (totalRevenue || 1)) * 100).toFixed(1)}%`
                  : "USD"}
              </span>
            </div>
          </div>
...
export default React.memo(KDChartCustomerRevenue);
