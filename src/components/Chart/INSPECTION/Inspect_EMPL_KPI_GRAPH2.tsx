...
import { KPI_NV_KIEMTRA } from "../../../pages/qc/interfaces/qcInterface";

const Inspect_EMPL_KPI_GRAPH2 = ({
  dldata,
}: {dldata: KPI_NV_KIEMTRA[]}) => {
    ...
    if (active && payload && payload.length) {      
      return (
        ...
        >
          <p>Nhân viên {label}:</p>                    
          <p className='label'>
            TOTAL_KIEMTRA_KPI: {`${(payload[0].payload.TOTAL_KIEMTRA_KPI)?.toLocaleString("en-US",{style:'percent',minimumFractionDigits: 0,maximumFractionDigits: 0})}`}
          </p>   
          <p className='label'>
            DIEM_CON: {`${(payload[0].payload.DIEM_CON)?.toLocaleString("en-US",{style:'percent',minimumFractionDigits: 0,maximumFractionDigits: 0})}`}
          </p>   
          <p className='label'>
...
export default Inspect_EMPL_KPI_GRAPH2;
