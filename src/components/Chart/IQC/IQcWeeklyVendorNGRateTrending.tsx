...
const IQcWeeklyVendorNGRateTrending = ({ dldata, onClick }: { dldata: IQC_VENDOR_NGRATE_DATA[], onClick: (e: any) => void }) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip">
          <p className="label">{`Tuần: ${label}`}</p>
...
// Tạo danh sách các tuần duy nhất
  const weeks = Array.from(new Set(dldata.map(item => item.INSPECT_YW))).sort();
// Tạo dữ liệu pivot cho Recharts
const chartData = weeks.map(week => {
  const dataPoint: { INSPECT_YW: string; [key: string]: string | number } = { INSPECT_YW: week || '' };
  dldata.forEach(item => {
    if (item.INSPECT_YW === week) {
...
export default IQcWeeklyVendorNGRateTrending;
