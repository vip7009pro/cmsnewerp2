...
const SX_EQ_KPI_GRAPH = ({
  dldata,
}: {dldata: LOSS_TIME_DATA_THEO_MAY[]}) => {
  ...
  return (
    <CustomResponsiveContainer>
       <ComposedChart
        width={500}
        height={300}
        data={dldata.filter((item: LOSS_TIME_DATA_THEO_MAY) => item.DAILY_KPI > 0)}
        margin={{
          top: 5,
          right: 30,
          left: 20,
      ...
      >
        ...
        <XAxis dataKey='EQ_NAME' height={40} tick={{ fontSize: '0.7rem' }}>
          <Label value='EQ_NAME' offset={0} position='insideBottom' style={{ fontWeight: 'normal', fontSize: '0.7rem' }} />
        </XAxis>
        <YAxis      
          yAxisId='left-axis'    
          label={{
...
export default SX_EQ_KPI_GRAPH;
