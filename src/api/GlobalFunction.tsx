...
export const SaveExcel = (data: any, title: string) => {
  ...
  const safeCellValue = (v: any) => {
    ...
    try {
      // stringify complex values into a single cell (still tabular)
  ...
  // Collect headers (union of keys) - allows exporting all columns.
  // If a particular row throws during key enumeration, we skip that row's keys but still export its known fields.
  ...
  for (let cci = 0; cci < colChunks.length; cci++) {
    ...
    // Split rows into multiple sheets by an upper bound on total cells per sheet.
    // This avoids creating a worksheet with millions of address keys (which triggers
    // "Too many properties to enumerate" in some JS engines / builds).
    const excelRowLimit = EXCEL_MAX_ROWS - 1; // keep 1 header row
    ...
    for (let start = 0; start < rows.length; start += rowsPerSheet) {
      ...
      // Use dense mode: worksheet is an array-of-arrays internally, not a giant object
      // with millions of properties.
